...
describe('Find Service (e2e)', ()=>{
    ...
    test('should be able to find a service', async()=>{
        ...
        const responseFindService = await request(fastifyApp.server)
        .get(`/api/services/${id}`)
        .set('Authorization', `Bearer ${accessToken}`)
        .send()

        expect(responseFindService.statusCode).toEqual(200)
    })

    test('should not be able to find a service with invalid id', async()=>{
        const {accessToken, user} = await createAndAuthenticateUser(
            fastifyApp,
            'ADMIN',
