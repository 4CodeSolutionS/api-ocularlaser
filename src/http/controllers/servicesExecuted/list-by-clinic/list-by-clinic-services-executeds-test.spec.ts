...
describe('List Service Executed by Clinic (e2e)', ()=>{
    beforeAll(async()=>{
    ...
    test('should be able to list a service executed by clinic', async()=>{
       ...
       const idAddress = '777eea13-3d79-4a39-a4a7-904e08affab7'
        ...
        const responseListByClinicServiceExecuted = await request(fastifyApp.server)
        .get(`/api/services-executeds/clinic?idClinic=${idClinic}&page=${page}`)
        .set('Authorization', `Bearer ${accessToken}`)
        .send()


        expect(responseListByClinicServiceExecuted.statusCode).toEqual(200)
    ...
    test('should not be able to list a service executed by user with invalid idClinic', async()=>{
        ...
        const page = 1
        const fakeIdClinic = '4977eeec-d965-4c18-8af8-ce4adbaca2fc'

        const responseListByUserServiceExecuted = await request(fastifyApp.server)
        .get(`/api/services-executeds/user?idUser=${fakeIdClinic}&page=${page}`)
        .set('Authorization', `Bearer ${accessToken}`)
        .send()
