...
describe('Find card by user (e2e)', ()=>{
    beforeAll(async()=>{
    ...
    test('should be able to find a card by user', async()=>{
        ...
        .get(`/api/cards/${card.idUser}`)
        .set('Authorization', `Bearer ${accessToken}`)
        .send()
        expect(responseCreateService.statusCode).toEqual(200)
    })

    test('should not be able to find a card with invalid idUser invalid', async()=>{
        const {accessToken, user} = await createAndAuthenticateUser(
            fastifyApp,
            'ADMIN',
